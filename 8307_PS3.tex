...
\begin{enumerate}[1.]
    ...
    \item
    Firm's value function:\\
    $V(z_t, n_{t-1}) = \max\limits_{n_t, X}\left[z_tn_t^\alpha -w_tn_t -\tau w_t \max(o, n_{t-1}-n_t) + \beta(1-X) E(V(z_{t+1},n_t)) -X \tau E(w_{t+1}) n_t\right]$\\
    $X \in \{1,0\}$\\
    Transition function of measure of types:\\
    $\mu_{t+1}(z', n') = \sum\limits_{z}\sum\limits_{n}\left[(1-X^*(z,n))\textbf{1}(n^*(z, n) = n')f(z'|z)\mu_t(z,n)\right] + M_t\psi(z)\textbf{1}(n'=0)$\\
    Where $n^*(z,n)$ and $X^*(z,n)$ are the argmax from the value function with arguments z and n. $\textbf{1}(a) = 1$ if a is true and $0$ otherwise. 
\end{enumerate}
